      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <select value={roleFilter} onChange={e => setRoleFilter(e.target.value)}
          ...
          {['member', 'gymOwner'].map(r => (
            <option key={r} value={r} className="bg-white capitalize">{r === 'gymOwner' ? 'Gym Owner' : 'Member'}</option>
          ))}
        </select>
      </div>
        ...
        <div className="divide-y divide-[#18181B]/6">
          {filtered.map(u => (
            <div key={u.id} className="grid grid-cols-12 px-5 py-3 items-center">
              ...
              <p className="col-span-2 text-xs text-[#18181B]/60">{(u.revenue / 1000000).toFixed(1)}M</p>
              <span className={`col-span-1 text-xs ${u.status === 'active' ? 'text-green-400' : 'text-red-400'}`}>{u.status}</span>
              <button onClick={() => toggleStatus(u.id)} className={`col-span-1 p-1.5 rounded-lg w-fit transition-colors ${u.status === 'active' ? 'text-[#18181B]/40 hover:text-red-400 hover:bg-red-400/10' : 'text-[#18181B]/40 hover:text-green-400 hover:bg-green-400/10'}`}>
