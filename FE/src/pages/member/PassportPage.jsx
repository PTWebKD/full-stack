...
const levels = [
  ...
  { name: 'Champion', minPts: 1500, color: '#00d4ff' },
  { name: 'Elite', minPts: 3000, color: '#f97316' },
  { name: 'Legend', minPts: 6000, color: '#a855f7' },
];

const badges = [
  { id: 1, name: 'First Rep', desc: 'Log your first workout', icon: Dumbbell, color: '#003a5a', earned: true, earnedAt: '2024-01-15' },
  { id: 2, name: '7-Day Streak', desc: 'Train 7 days in a row', icon: Flame, color: '#f97316', earned: true, earnedAt: '2024-02-10' },
  { id: 3, name: '100 Workouts', desc: 'Complete 100 sessions', icon: Trophy, color: '#fbbf24', earned: true, earnedAt: '2024-12-28' },
  { id: 4, name: 'PR Crusher', desc: 'Set 5 personal records', icon: Zap, color: '#00d4ff', earned: true, earnedAt: '2025-01-30' },
...
export default function PassportPage() {
  ...
  return (
    <div className="space-y-6 max-w-3xl mx-auto">
      ...
      {/* Badges */}
      <div className="glass rounded-2xl p-5 border border-white/5">
        <h3 className="font-semibold text-white mb-4">Badges <span className="text-xs text-white/30 ml-1">({badges.filter(b => b.earned).length}/{badges.length} earned)</span></h3>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {badges.map(b => (
            <div key={b.id} className={`rounded-xl p-4 text-center border transition-all ${b.earned ? 'border-white/10 bg-white/[0.03]' : 'border-white/5 opacity-40'}`}>
              <div className="w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-2" style={b.earned ? { background: `${b.color}15`, border: `1px solid ${b.color}30` } : { background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.05)' }}>
                {b.earned ? <b.icon className="w-5 h-5" style={{ color: b.color }} /> : <Lock className="w-4 h-4 text-white/20" />}
              </div>
              <p className="text-xs font-semibold text-white mb-0.5">{b.name}</p>
              <p className="text-xs text-white/30 leading-tight">{b.desc}</p>
              {b.earned && b.earnedAt && <p className="text-xs text-white/20 mt-1">{b.earnedAt}</p>}
            </div>
          ))}
        </div>
      </div>
