...
export default function SessionDetailPage() {
  const { id } = useParams();
  const session = mockWorkoutHistory.find(w => w.id === Number(id));
  ...
  const totalReps = session.exercises.reduce((s, e) => s + e.sets.reduce((ss, set) => ss + set.reps, 0), 0);
  ...
  return (
    <div className="max-w-2xl space-y-5 mx-auto">
      ...
      <div className="glass rounded-2xl p-5 border border-white/5">
        <h2 className="text-xl font-black text-white mb-1">{session.name}</h2>
        <p className="text-white/40 text-sm mb-4">{session.date}</p>
        <div className="grid grid-cols-3 gap-3">
          {[
            { label: 'Duration', value: `${session.duration}m`, icon: Clock },
            { label: 'Volume', value: `${(session.volume / 1000).toFixed(1)}T`, icon: Weight },
