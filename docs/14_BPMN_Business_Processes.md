...
Pool: MEMBER
  [Start] -> [Check-in QR] -> {Goi tap con hieu luc?}
    ...
    Co -> [Ghi CHECK_INS] -> [Tao buoi tap moi]
       ...
       -> {Ket thuc buoi tap?}
           Khong -> (tiep tuc log)
           Co -> [Tong hop XP, streak (+1 toi da 1 lan/ngay — BR-23), cap nhat Fitness Passport]
...
```
Pool: MEMBER
  -- Luong 0: Goal Onboarding (chi chay 1 lan dau, neu chua co TRANSFORMATION_GOALS active) --
  [Mo /journey] -> [B1: Chon muc tieu] (muscle_gain/fat_loss/maintain/strength)
  -> [B2: Nhap chi tieu + deadline]
  -> [B3: Chon so buoi/tuan] (2/3/4/5 ngay)
  -> [B4: Chon trinh do] (beginner/intermediate/advanced)
  -> [B5: He thong goi y 2-3 chuong trinh phu hop (BR-41)]
  -> [Member chon 1 chuong trinh]
  -> [Tao TRANSFORMATION_GOALS + MEMBER_PROGRAMS.status='active'] -> (vao Luong chinh)

  -- Luong chinh: Tap theo ngay --
  [Mo /journey] -> {Da chon Lịch tập?}
      Co -> [Hien thi goi y ngay tap theo lich (Tuan X, Buoi Y)]
         -> {Tap theo lich hay chon nhom co?}
               Theo lich -> [Bam BAT DAU TAP THEO CHUONG TRINH] -> (tu dong chon nhom co)
               Tu chon -> [CHON NHOM CO HOM NAY] (Chan / Nguc / Lung + Vai / Toan than / Tu chon)
      Khong -> [CHON NHOM CO HOM NAY] (Chan / Nguc / Lung + Vai / Toan than / Tu chon)
...
Pool: GYM OWNER
  ...
  -> {Loai rec?}
      R7 -> [Goi dien check-in member bo tap] -> [Ghi ket qua### 3.3.3 — Quy trình Delivery & Quản lý đơn hàng
...
4. Sơ đồ BPMN (mô tả văn bản với Subprocess Hộp đen)

```
Pool: MEMBER
  [Chon "Giao hang"] -> [Chon SHIPPING_ADDRESSES]
  -> [Thanh toan online — goi SP-01] -> SUCCESS
  -> [Tao don, status='pending'] -> [Theo doi status tren Web App]
  -> {Muon huy don?}
       Co (khi pending/preparing) -> [Huy don] -> [FitCoin mo khoa & Hoan tien] -> [End]
       Khong -> [Cho nhan hang] -> [Nhan hang tu Shipper] -> [Done] -> [End]

Pool: GYM OWNER
  [Nhan don moi tai /gym-owner/orders]
  -> [Chuan bi san pham] -> [Xac nhan san sang]
  -> [Kich hoat Subprocess: QUY TRÌNH GIAO HÀNG BÊN THỨ 3 (+)] 
     (Bàn giao cho đơn vị vận chuyển GHN/Ahamove xử lý vật lý)
  -> [Nhan phan hoi trang thai tu Subprocess] -> [End]

Pool: COLLAPSED SUBPROCESS (Hộp đen - Bên thứ 3)
  [QUY TRÌNH GIAO HÀNG BÊN THỨ 3 (+)]
  (Nhiệm vụ: GHN/Ahamove điều phối shipper -> lấy hàng -> giao hàng -> gửi webhook báo kết quả)

Pool: HE THONG FITFUEL+
  Nhan tin hieu tu Gym Owner -> Doi status = 'preparing'
  Nhan tin hieu tu Subprocess (+) ->
    Doi status sang 'shipped' -> 'delivering' -> 'done' -> Gui NOTIFICATIONS cho Member
```
