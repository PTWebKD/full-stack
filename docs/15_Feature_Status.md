...
**Biểu đồ tiến độ tập luyện (Exercise Progress)** — đường tiến bộ mức tạ/khối lượng theo thời gian.
Quy trình dự định: chọn 1 bài tập → hệ thống lấy toàn bộ `EXERCISE_LOGS` của bài đó theo thời gian → vẽ biểu đồ đường thể hiện mức tạ/volume tăng dần.
Trạng thái: mock/tĩnh, chưa có API lịch sử phù hợp để vẽ biểu đồ động.
